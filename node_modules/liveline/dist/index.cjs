...
function drawLine(ctx, layout, palette, visible, smoothValue, now, showFill, scrubX, scrubAmount = 0, chartReveal = 1, now_ms = 0, colorBlend = 1, skipDashLine = false, fillScale = 1, extendToNow = true) {
    ...
    if (isCandle) {
      ...
      for (const s of effectiveMultiSeries) {
        ...
        if (visible.length >= 2) {
          ...
          seriesEntries.push({ visible, smoothValue: sv, palette: s.palette, label: s.label, alpha, extendToNow: s.extendToNow !== false });
        }
      }
