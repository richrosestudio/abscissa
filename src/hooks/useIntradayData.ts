...
export function useIntradayData(holdings: Holding[], timeRange: TimeRange = '1D') {
  ...
  const fetchData = useCallback(async (signal: AbortSignal) => {
    ...
    try {
      ...
      for (const h of holdings) {
        const entry = json.series?.[h.id]
        if (!entry || entry.error) continue
        const points = (entry.points as { t: number; pct: number }[]).map(p => ({
          time: p.t,
          value: p.pct,
        }))
      ...
      // If the API returned no usable data for any symbol, fall back to mock
      ...
      if (signal.aborted) return
      // Fall back to mock data so the UI stays useful during development
      ...
      setState(s => ({
        ...
        data: Object.keys(s.data).length > 0 ? s.data : mockData,
  ...
  const initMock = useCallback((signal: AbortSignal) => {
    ...
    setState(s => ({ ...s, data: mockData, loading: false, error: null, usingMock: true }))
  }, [holdings])
  ...
  const tickMock = useCallback(() => {
    ...
    setState(s => ({ ...s, data: next, usingMock: true }))
  }, [holdings])
  ...
  const fetchHistorical = useCallback(async (signal: AbortSignal) => {
    ...
    try {
      ...
      setState({ data: nextData, loading: false, error: null, lastFetchAt: Date.now(), usingMock: false })
    } catch {
      if (signal.aborted) return
      // Fall back to generated mock historical data
      ...
      setState(s => ({ ...s, loading: false, data: Object.keys(s.data).length > 0 ? s.data : mockData, usingMock: Object.keys(s.data).length === 0 }))
    }
  }, [holdings, timeRange])
